        ...
        if self.fall_time >= current_fall_speed:
            ...
            if not self.board1.game_over:
                if not self.board1.move(0, 1):
                    ...
                    # 如果消除了行，给对手添加垃圾行
                    if lines_cleared > 0:
                        self.board2.add_garbage_lines(min(lines_cleared, 2))  # 最多添加2行
            ...
            if not self.board2.game_over:
                if not self.board2.move(0, 1):
                    ...
                    # 如果消除了行，给对手添加垃圾行
                    if lines_cleared > 0:
                        self.board1.add_garbage_lines(min(lines_cleared, 2))  # 最多添加2行
